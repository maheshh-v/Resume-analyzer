...
async def test_full_interview_flow_strong_then_strong_completes(client, fake_provider):
    ...
    state = (await client.post(f"/interview/{token}/questions/{q1_id}/answer", json={"answer_text": "We hit input-shape retracing on every batch..."})).json()
    assert state["is_complete"] is False
    ...
    state = (await client.post(f"/interview/{token}/questions/{q2_id}/answer", json={"answer_text": "I owned a blue-green rollout across 12 nodes..."})).json()
...
async def test_weak_answer_triggers_one_followup_then_advances(client, fake_provider):
    ...
    state = (await client.post(f"/interview/{token}/questions/{q1_id}/answer", json={"answer_text": "TensorFlow is a machine learning framework."})).json()
...
async def test_interview_creation_fails_when_resume_still_processing(client, fake_provider):
    ...
    resp = await client.post(f"/candidates/{candidate_id}/interviews")
...
async def test_public_interview_route_requires_no_auth(client, fake_provider):
    ...
    fake_provider.responses.append(_question("Tell me about a TensorFlow retracing bug you hit.", ["retracing"]))
    resp = await client.get(f"/interview/{token}")
    assert resp.status_code != 401


@pytest.mark.asyncio
async def test_unknown_token_returns_404(client):
    resp = await client.get("/interview/not-a-real-token")
...
async def test_recruiter_transcript_reflects_qa_exchange(client, fake_provider):
    candidate_id = await _setup_candidate_with_two_unverified_must_haves(client, fake_provider)
    interview_resp = await client.post(f"/candidates/{candidate_id}/interviews")
    ...
    fake_provider.responses.append(_evaluation("strong", "named the retracing bug precisely"))
    fake_provider.responses.append(_question("Walk me through a k8s rollout you owned.", ["rollout"]))
    await client.post(f"/interview/{token}/questions/{q1_id}/answer", json={"answer_text": "We hit input-shape retracing on every batch..."})

    transcript_resp = await client.get(f"/candidates/{candidate_id}/interviews/{interview_id}/transcript")
